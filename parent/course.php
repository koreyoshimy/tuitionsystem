...
// Error messages
$errorMessages = [
    'duplicate' => 'This child already has this subject booked. Please choose a different subject.',
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // ===== PASTE THIS (DUPLICATE CHECK) =====
    // Check if child already has this subject
    $checkQuery = "SELECT * FROM subjects WHERE username = ? AND subject_name = ?";
    $checkStmt = $conn->prepare($checkQuery);
    $checkStmt->bind_param("ss", $child_id, $subject_name);
    $checkStmt->execute();
    $checkResult = $checkStmt->get_result();
    
    if ($checkResult->num_rows > 0) {
        header("Location: course.php?error=duplicate");
        exit();
    }
    // ===== END OF PASTE =====
...
$subject_id = $subjectsMap[$subject_name] ?? '0000';
    $subjectSchedules = [
        'Mathematics' => [
...
<?php if (isset($_GET['success'])): ?>
    <div style="color: green; padding: 10px; margin: 10px 0; border: 1px solid green; border-radius: 5px;">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Modify the nextStep function to validate before proceeding
function nextStep() {
    if (currentStep < totalSteps) {
        // Validate current step before proceeding
        let canProceed = true;
        
        if (currentStep === 1 && !formData.child_id) {
            canProceed = false;
            alert('Please select a child');
        }
        else if (currentStep === 2 && !formData.subject) {
            canProceed = false;
            alert('Please select a subject');
        }
        else if (currentStep === 3 && !formData.timeslot) {
            canProceed = false;
...
// Add this to preserve form data if there was an error
document.addEventListener('DOMContentLoaded', function() {
    <?php if (isset($_SESSION['form_data'])): ?>
        // Repopulate form data from session
        const formData = <?php echo json_encode($_SESSION['form_data']); ?>;
        
        document.getElementById('childSelect').value = formData.child_id;
        // Trigger change to populate other fields
        document.getElementById('childSelect').dispatchEvent(new Event('change'));
        
        // Select subject after a delay (to allow DOM update)
        setTimeout(() => {
            const subjectCards = document.querySelectorAll('#subjectOptions .option-card');
            subjectCards.forEach(card => {
                if (card.dataset.value === formData.subject) {
                    card.click();
                }
            });
            
            // Select timeslot after another delay
            setTimeout(() => {
                const timeslotCards = document.querySelectorAll('#timeslotOptions .option-card');
                timeslotCards.forEach(card => {
                    if (card.dataset.timeslot === formData.timeslot) {
                        card.click();
                    }
                });
            }, 100);
...
document.querySelectorAll('#subjectOptions .option-card').forEach(card => {
    card.addEventListener('click', function () {
        ...
        subjectSchedules[subject].forEach(slot => {
            ...
            `;
            div.dataset.timeslot = `${slot.day} ${slot.time}`;
            div.addEventListener('click', function () {
                document.querySelectorAll('#timeslotOptions .option-card').forEach(c => c.classList.remove('selected'));
                div.classList.add('selected');
                formData.timeslot = div.dataset.timeslot;
                document.getElementById('selectedTimeslot').value = formData.timeslot;
                nextBtn.disabled = false;
            });
            timeslotContainer.appendChild(div);
        });

        nextBtn.disabled = true; // Reset "Next" button in step 3 until timeslot selected
        nextStep();
    });
});
// In your JavaScript, add this at the end to preserve selections if there was an error
document.addEventListener('DOMContentLoaded', function() {
    <?php if (isset($_POST['child_id'])): ?>
        document.getElementById('childSelect').value = '<?php echo $_POST["child_id"]; ?>';
        formData.child_id = '<?php echo $_POST["child_id"]; ?>';
        // Trigger change event to populate other fields
        document.getElementById('childSelect').dispatchEvent(new Event('change'));
    <?php endif; ?>
    
    <?php if (isset($_POST['subject'])): ?>
        // Select the subject card
        const subjectCards = document.querySelectorAll('#subjectOptions .option-card');
        subjectCards.forEach(card => {
            if (card.dataset.value === '<?php echo $_POST["subject"]; ?>') {
                card.click();
            }
        });
    <?php endif; ?>
    ...
    <?php if (isset($_POST['timeslot'])): ?>
        // Need to wait a bit for timeslots to populate
        setTimeout(() => {
            const timeslotCards = document.querySelectorAll('#timeslotOptions .option-card');
            timeslotCards.forEach(card => {
                if (card.dataset.value === '<?php echo $_POST["timeslot"]; ?>') {
                    card.click();
                }
            });
        }, 100);
